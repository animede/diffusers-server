...
- **真横(`left` / `right`)は2026-07-29に追加**(image-3d が side view に対応したため)。
  旧記録の「Tポーズの真横投影は6通りすべてで破綻する」は現行パイプラインでは覆った
  (破綻しない)。ただし**角度で指示すると3/4止まり**になるため、
  「鼻が画面端を向く / 見える目と耳は片方だけ / 胴体はエッジオン / 両腕は強く短縮され
  肩と胴体の陰にほぼ隠れる」と**“見えるもの”で指示**している(実測: 被写体bboxの
  幅/高さ比が 角度指示 0.56〜0.84 に対し 0.38〜0.52。正面のTポーズは 0.97〜1.00)。
  **腕の短縮の度合いは seed で変わる**ので、腕が長く見えるときは seed を変えて
  生成し直すこと。**image-3d へ渡すのは front / back / left / right の4枚**
